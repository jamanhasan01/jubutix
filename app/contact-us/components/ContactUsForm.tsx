...
const Title = ({
  ...
  <div className={classname}>
    <h1 className='text-3xl md:text-4xl font-bold tracking-tighter text-gray-900 dark:text-gray-50'>
      {title}
    </h1>
...
const ContactItem = ({ icon: Icon, text }: { icon: React.ElementType; text: string }) => (
  <div className='flex items-center justify-center md:justify-start gap-4'>
    ...
    <span className='text-lg text-gray-800 dark:text-gray-200 text-left'>{text}</span>
  </div>
)
          ...
          <div className='text-center md:text-left'>
            ...
            <BlurFade delay={0.45} inView>
              ...
              <div className='mt-8 space-y-4'>
                ...
                <ContactItem
                  icon={MapPin}
                  text='343/2 Choto Alampur, Debidwar, Comilla, Bangladesh'
                />
              </div>
            </BlurFade>
                ...
                {socialLinks.map((social) => (
                  ...
                  >
                    <Button
                      variant='outline'
                      size='icon'
                      className='group-hover:bg-accent transition-colors hover:cursor-pointer'
                    >
                      <social.icon
                        className={`h-5 w-5 ${social.color} group-hover:opacity-80 transition-opacity`}
                      />
                    </Button>
                  </a>
                ))}
